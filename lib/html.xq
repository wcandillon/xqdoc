...
module namespace html = "http://www.28msec.com/xqdoc/lib/html";
    ...
    }
    {
      ...
      return
        if(exists($sees)) then
          ...
          <ul>{
            for $see in $sees
            let $see := if(exists($see/a) and exists($see/text())) then
                          $see/node()
                        else if(exists($see/node()) and empty($see/a)) then
                          <a href="{$see/node()}">{$see/node()}</a>
                        else if(exists($see/a)) then
                          <a href="{$see/a/text()}">{$see/a/text()}</a>
                        else ""
            return <li>{$see}</li>
          }</ul>
          </section>
